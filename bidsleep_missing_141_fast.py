...
def recover(index: int, count: int, completed_path: Path, output: Path) -> None:
    ...
    try:
        ...
        if len(completed) != 112:
            raise RuntimeError(f"Expected 112 completed pairs, observed {len(completed)}")
        if len(missing) != 141:
            raise RuntimeError(f"Expected 141 missing pairs, observed {len(missing)}")
        ...
        pd.DataFrame(failures, columns=["subject_id", "night_index", "error_type", "error"]).to_csv(
        ...
        if failures:
            raise SystemExit(f"Fast recovery shard {index} had {len(failures)} failures")
    finally:
        archive.close()
        buffered.close()
